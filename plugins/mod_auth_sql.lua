...
local function get_password(username)
		--local stmt, err = getsql("SELECT `szStudentId` FROM `sch_student` WHERE `szStudentId`=? AND `realm`=?", username, module.host);
	module:log("debug", "get_password ?", username);
	local stmt, err = getsql("SELECT `szStudentId` FROM `sch_student` WHERE `szStudentId`=?", username);
	if stmt then
		for row in stmt:rows(true) do
			module:log("debug", "Student Type");
			return row.szStudentId;
		end
	end

	stmt, err = getsql("SELECT `szId` FROM `sch_lecturer` WHERE `szId`=?", username);
	if stmt then
		for row in stmt:rows(true) do
			module:log("debug", "Teacher Type");
			return row.szId;
		end
	end
end
...
provider = {};
function provider.get_usertype(username)
...
function provider.users()
	local stmt, err = getsql("SELECT `szStudentId` FROM `sch_student`");
	if stmt then
		...
		return function()
			...
			stmt, err = getsql("SELECT `szId` FROM `sch_lecturer`");
			next, state = stmt:rows(true);
			for row in next, state do
				return row.szId
			end
		end
